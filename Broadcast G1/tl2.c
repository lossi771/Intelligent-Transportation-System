...
int cross_situation[2]={2,2}; //first element main street, second element second street
...
PROCESS(tl2_control_process, "TL2_CONTROL_PROCESS");
AUTOSTART_PROCESSES(&tl2_control_process);
...
static void broadcast_recv(struct broadcast_conn *c, const linkaddr_t *from){
	if (started==false){
		process_post(&tl2_control_process,START_TOGGLING,packetbuf_dataptr());
		started = true;
	}
	else{
...
PROCESS_THREAD(tl2_control_process, ev, data) {
	...
	PROCESS_WAIT_EVENT_UNTIL(START_TOGGLING);
	etimer_set(&toggle_timer,CLOCK_SECOND);
	...
	while(1){
		...
		if(etimer_expired(&battery_blinking_timer)){
			...
			else if(etimer_expired(&toggle_timer) && crossing_vehicle==false){
